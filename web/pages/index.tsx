...
import { latestProjectsQuery, siteSettingsQuery } from "@web/lib/queries";
import { sanityClient, urlFor } from "@web/lib/sanity";
import pageStyles from "@web/styles/home.module.scss";
import previewStyles from "@web/styles/projectPreview.module.scss";

export default function HomePage({ site, projects }: { site: any; projects: any[] }) {
  const ogImage = site?.portrait?.asset
    ? urlFor(site.portrait)?.width(1200).height(630).fit("crop").url()
    : null;
  return (
    <SiteLayout
