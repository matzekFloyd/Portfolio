...
import { blocksToText, PortableTextContent } from "@web/lib/portableText";
import { projectBySlugQuery, projectSlugsQuery, siteSettingsQuery } from "@web/lib/queries";
import { sanityClient, urlFor } from "@web/lib/sanity";
import styles from "@web/styles/projectDetail.module.scss";

function isExternalHttpUrl(value: unknown) {
...
function buildMetaImage(image: any, fallbackAlt: string) {
...
export default function ProjectPage({ site, project }: { site: any; project: any }) {
  const [lightboxImage, setLightboxImage] = useState<any>(null);
  if (!project)
    return (
      <SiteLayout siteTitle={site?.title} pageTitle="Project" description={site?.description}>
        Not found.
      </SiteLayout>
    );
  ...
  ].filter(Boolean) as Array<{ src: string; alt: string; caption: string | null }>;
  ...
  return (
    ...
    >
      ...
      <div
        className={`${styles.contentGrid}${!hasMetaColumn ? ` ${styles.contentGridSingle}` : ""}`}
      >
                    ...
                    <li key={category._id}>
                      {linkUrl ? (
                        <a
                          ...
                          aria-label={
                            openInNewTab ? `${category.title} (opens in new tab)` : category.title
                          }
                        >
                          {category.title}
                        </a>
...
export async function getStaticPaths() {
  ...
  return {
    paths: (slugs || []).map((item: any) => ({ params: { slug: item.slug } })),
    fallback: false,
  };
}
